...
export default function CardSearch({ category }: { category: string }) {
  ...
  const { data, error, isLoading } = useSWR(
    `/api/posts?filters[judul][$containsi]=${category}&pagination[page]=${pageNumber}&pagination[pageSize]=${pageSize}&sort[0]=createdAt:desc&&populate=*`,
    filterSubBerita,
    {
  ...
  return (
    <div className="flex flex-col gap-5 p-5 w-full">
      ...
      {data.data.map((item: any) => {
        ...
        return (
          <div key={item.id} className="flex  md:h-40 w-full justify-center">
